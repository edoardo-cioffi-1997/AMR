...
{	
	...
	ros::NodeHandle nh("~");
	nh.param("rate", rate, 0.5);
	nh.param<std::string>("destination_frame", destination_frame, "amr_body");
	...
	nh.param("angle_min", angle_min, -3.141592);
	nh.param("angle_max", angle_max, 3.141592);
	nh.param("angle_increment", angle_increment, 0.0058);
	nh.param("scan_time", scan_time, 0.0333333);
	nh.param("range_min", range_min, 0.0);
	nh.param("range_max", range_max, 4.0);

	//ROS_INFO("Parameters Loaded: rate = %f, scan_destination_topic = %s", rate, scan_destination_topic.c_str());

    // ros::Rate loop_rate(rate);

    // // Try to detect topics
    // while (!laserscan_topic_parser()) {
    //     ROS_DEBUG("LaserscanMerger: Retrying to detect the topics to subscribe to...");
    //     loop_rate.sleep();
    // }
	// ROS_INFO("Topics detected and subscribed.");
